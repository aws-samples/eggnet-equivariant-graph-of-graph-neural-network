...
if __name__ == '__main__':
    ...
    for line in f.readlines()[1:]: # if the file has headers and pay attention to the columns (whether have peptide binding site labels)
        prot, pep, label, pep_ss, prot_ss  = line.strip().split('\t')
        pep_set.add(pep)
        prot_set.add(prot)
        pep_ss_set.add(pep_ss)
        prot_ss_set.add(prot_ss)

    f.close()
    pep_len = [len(pep) for pep in pep_set]
    prot_len = [len(seq) for seq in prot_set]
    pep_ss_len = [len(pep_ss) for pep_ss in pep_ss_set]
    prot_ss_len = [len(seq_ss) for seq_ss in prot_ss_set]
    ...
    print('prot_set', len(prot_len), 'pad_prot_len', pad_prot_len)
    print('num of peptide ss', len(pep_ss_len), 'pad_pep_len', pad_pep_len)
    print('prot_ss_set', len(prot_ss_len), 'pad_prot_len', pad_prot_len)
    ...
    with open('./dense_feature_dict/Protein_pssm_dict', 'rb') as f: # value: (sequence_length, 20) without sigmoid
        protein_pssm_dict = pickle.load(f)

    with open('./dense_feature_dict/Protein_Intrinsic_dict', 'rb') as f: # value: (sequence_length, 3): long, short, anchor
        protein_intrinsic_dict = pickle.load(f)

    with open('./dense_feature_dict/Peptide_Intrinsic_dict_v3', 'rb') as f: # value: (sequence_length, 3): long, short, anchor
    ...
    for line in f.readlines()[1:]:
        ...
        if prot not in protein_feature_dict:
            feature = label_sequence(prot, pad_prot_len, amino_acid_set)
            protein_feature_dict[prot] = feature
        ...
        if prot_ss not in protein_ss_feature_dict:
            feature = label_seq_ss(prot_ss, pad_prot_len, seq_ss_set)
            protein_ss_feature_dict[prot_ss] = feature
        ...
        if prot not in protein_2_feature_dict:
            feature = label_sequence(prot, pad_prot_len, physicochemical_set)
            protein_2_feature_dict[prot] = feature
        ...
        if prot not in protein_dense_feature_dict:
            feature_pssm = padding_sigmoid_pssm(protein_pssm_dict[prot], pad_prot_len)
            feature_intrinsic = padding_intrinsic_disorder(protein_intrinsic_dict[prot], pad_prot_len)
            feature_dense = np.concatenate((feature_pssm, feature_intrinsic), axis=1)
            protein_dense_feature_dict[prot] = feature_dense
        if prot not in protein_intrinsic_feature_dict:
            feature_intrinsic = padding_intrinsic_disorder(protein_intrinsic_dict[prot], pad_prot_len)
            protein_intrinsic_feature_dict[prot] = feature_intrinsic
