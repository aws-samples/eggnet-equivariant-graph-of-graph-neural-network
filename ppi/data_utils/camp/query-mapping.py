# -*- coding: utf-8 -*-
pepbdb_inputdir = '/home/ec2-user/SageMaker/efs/data/CAMP/paper/' #tar -xvzf'd version of pepbdb-20200318
...
for pid in querys:
        ...
        address=os.path.join(pepbdb_inputdir, 'pepbdb/', pdbid, 'peptide.pdb')#receptor.pdb
...
sorted_lists={}
pass_lists=[]
print("Total queryes {0}".format(len(querys)))
querys = [q for q in querys if q in whole_dict.keys()]
print("Queries with pdb data {0}, filtered to those with pdb data locally {1}".format(len(whole_dict.keys()), len(querys)))

with open('step2/peptide-mapping.txt','w') as f2:#prot-mapping.txt
    for pdbid in querys:
...
with open('step2/query_peptide_sequence_index.txt', 'w') as f3:      
