...
raw_str=''
inputdir='/home/ec2-user/SageMaker/efs/data/CAMP/'
with open(inputdir + 'pdb_seqres.txt','r') as f:
    for line in f.readlines():
        raw_str = raw_str+line.replace('\n','###').replace('->', 'to').replace('<1>', '-1-').replace('<2>', '-2-')
...
df_fasta_raw.to_csv('step1/pdbid_all_fasta', encoding='utf-8', index=False, sep = '\t')
df_fasta.to_csv('step1/pdb_pep_chain', encoding='utf-8', index=False, sep = '\t')
...
def load(pdb_pep_dataset,plip_result_filename): #pdb_pep_chain   #plip_predict_result
    ...
    import random
    list_indices = list(range(df_fasta_pep.shape[0]))
    #random.shuffle(list_indices)
    outdir = '/home/ec2-user/SageMaker/efs/data/CAMP/processed/step1-peptide_result/'
    for i in list_indices[0:100]:
        pdb_id = df_fasta_pep['PDB_id'][i]
        chain = df_fasta_pep['chain'][i]
        #result_file_name = './peptide_result/'+pdb_id + '_'+chain+'_result.txt'
        result_file_name = outdir +pdb_id + '_'+chain+'_result.txt'
        try:
            ...
            pass # COMMENT THIS IF YOU WANT TO GET MORE PREDICTIONS
            print('Found no file for',pdb_id); print("retrieving from PLIP tool")
            # Add --nohydro to output more determinative predictions
            dockercmd = '/usr/bin/docker run --rm -v ${PWD}/peptide_result/:/peptide_result/ -w /peptide_result/ -u $(id -u ${USER}):$(id -g ${USER}) pharmai/plip:latest -i ' + pdb_id + ' -yv -t --name ' + pdb_id + '_'+chain+'_result'
            p = subprocess.run(dockercmd, shell=True)
            p = subprocess.run('rm ${PWD}/peptide_result/*.pdb', shell=True)
            p = subprocess.run('rm ${PWD}/peptide_result/*.pse', shell=True)
            #print(i,pdb_id,line)
    print('finish loading!')
    print('-----------------------------------------------------')
    print(df_predict.info())
    print('Left with {0}'.format(df_predict.shape))
...
def map_uniprot_chain(input_dataset,pdb_chain_uniprot_file): #df_predict_det1 #pdb_chain_uniprot.tsv
    df_sifts = pd.read_csv(pdb_chain_uniprot_file, sep = '\t', header = 0, comment='#')
    ...
    file_name = "step1/pdb_chain_uniprot-filtIDs.tsv"
...
def load_uni_seq(input_dataset,uniprot2seq_file=None):
...
df_predict = load('step1/pdb_pep_chain','step1/plip_predict_result')
print(df_predict.head())
df_predict_det1 = load_all_fasta('step1/pdbid_all_fasta',df_predict)
df_predict_det2_no_uni,df_predict_det3 = map_uniprot_chain(df_predict_det1,'step1/pdb_chain_uniprot.tsv') 

print(df_predict_det3.head())
print(df_predict_det2_no_uni.shape, df_predict_det3.shape)
file_name = "step1/pdb_chain_uniprot-processed.tsv"
df_predict_det3.to_csv(file_name, encoding = 'utf-8', index = False, sep = '\t')

#df_pdb_pairs = load_uni_seq(df_predict_det3,'step1/uniprot2seq')
