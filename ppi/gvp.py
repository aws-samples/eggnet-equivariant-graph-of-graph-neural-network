...
class GVPConv(nn.Module):
    ...
    def forward(self, g):
        g.update_all(
            message_func=self.message_udf, reduce_func=self.reduce_udf
        )
        return g.ndata["node_s_agg"], g.ndata["node_v_agg"]
    ...
    def message_udf(self, edges):
        """
        message function for GVP-GNN
        :param edges: EdgeBatch
        :return dict[str, tensor]: s_m: scalar message; s_v: vector message
        """
        s_i, v_i = edges.src["node_s"], edges.src["node_v"]
        s_j, v_j = edges.dst["node_s"], edges.dst["node_v"]
        edge_attr = edges.data["edge_s"], edges.data["edge_v"]

        s_m, v_m = self.message(s_i, v_i, s_j, v_j, edge_attr)
        return {"s_m": s_m, "v_m": v_m}

    def reduce_udf(self, nodes):
        """
        reduce function for GVP-GNN
        :param nodes: NodeBatch
        """
        s_m, v_m = nodes.mailbox["s_m"], nodes.mailbox["v_m"]

        return {
            "node_s_agg": torch.mean(s_m, dim=1),
            "node_v_agg": torch.mean(v_m, dim=1),
        }
...
class GVPConvLayer(nn.Module):
    ...
    def forward(self, g):
        ...
        x = g.ndata["node_s"], g.ndata["node_v"]
