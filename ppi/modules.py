...
class GVPMultiStageEnergyModel(nn.Module):
    """GVP-GNN Model (structure-only) modified from `MQAModel`:
    https://github.com/drorlab/gvp-pytorch/blob/main/gvp/model.py
    Inductive components: https://github.com/ACE-KAIST/PIGNet/blob/main/models.py
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Returns:
            None
        """
        super(GVPMultiStageEnergyModel, self).__init__()
        self.residual = residual
        self.num_outputs = num_outputs
        self.seq_embedding = seq_embedding
        self.vdw_N = vdw_N
        ...
        self.cal_vdw_interaction_A = nn.Sequential(
            nn.Linear(complex_node_h_dim, complex_node_h_dim),
            nn.ReLU(),
            nn.Linear(complex_node_h_dim, 1),
            nn.Sigmoid(),
        )
    ...
    def forward(self, protein_batch, ligand_batch, complex_batch, sample_batch, DM_min=0.5, cal_der_loss=False):
        """Perform the forward pass.
        Args:
            batch: dgl.DGLGraph
        Returns:
